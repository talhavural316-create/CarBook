...
namespace CarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminCategoryController : Controller
    {
        ...
        public async Task<IActionResult> CreateCategory(CreateCategoryDto createCategoryDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7014/api/Categories", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> RemoveCategory(int id)
        {
            ...
            if (!responseMessage.IsSuccessStatusCode)
            {
                responseMessage = await client.DeleteAsync($"https://localhost:7014/api/Categories?id={id}");
            }
        ...
        public async Task<IActionResult> UpdateCategory(UpdateCategoryDto updateCategoryDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7014/api/Categories", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminCategory", new { area = "Admin" });
            }
