...
namespace CarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminTestimonialController : Controller
    {
        ...
        public async Task<IActionResult> CreateTestimonial(CreateTestimonialDto createTestimonialDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7014/api/Testimonials", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7014/api/Testimonials?id={id}");
            if (!responseMessage.IsSuccessStatusCode)
            {
                responseMessage = await client.DeleteAsync($"https://localhost:7014/api/Testimonials/{id}");
            }
        ...
        public async Task<IActionResult> UpdateTestimonial(UpdateTestimonialDto updateTestimonialDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7014/api/Testimonials", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminTestimonial", new { area = "Admin" });
            }
